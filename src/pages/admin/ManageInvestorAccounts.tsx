...
const ManageInvestorAccounts = () => {
  ...
  // Filter users by investor role
  const investorUsers = useMemo(() => {
    return users.filter(user => user.role === 'investor');
  }, [users]);

  // Apply search and status filters
  const filteredUsers = useMemo(() => {
    let filtered = investorUsers;
    ...
    return filtered;
  }, [investorUsers, searchTerm, statusFilter]);

  // Calculate stats
  const stats = useMemo(() => {
    const active = investorUsers.filter(user => {
      const lastActive = user.last_active ? new Date(user.last_active) : null;
      const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    ...
    return {
      total: investorUsers.length,
      active,
      inactive: investorUsers.length - active
    };
  }, [investorUsers]);
  ...
  const formatLastActive = (date: string | null) => {
    ...
    const now = new Date();
    const diffInDays = Math.floor((now.getTime() - lastActive.getTime()) / (1000 * 60 * 60 * 1000));
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="text-center">Loading investor accounts...</div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Manage Investor Accounts</h1>
          <p className="text-gray-600 mt-2">Manage and monitor all investor user accounts</p>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Investor Accounts ({filteredUsers.length})</CardTitle>
            <CardDescription>
              Manage investor user accounts and their access
            </CardDescription>
          </CardHeader>
          <CardContent>
...
export default ManageInvestorAccounts;